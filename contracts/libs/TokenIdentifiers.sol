...
library TokenIdentifiers {
    ...
    uint256 constant SUPPLY_MASK = ((uint256(1) << SUPPLY_BITS) - uint256(1));
    uint256 constant INDEX_MASK =  (uint256(1) << INDEX_BITS) - uint256(1);
    ...
    function tokenCreator(uint256 _id) internal pure returns (address) {
        return address(uint160(_id >> (SUPPLY_BITS + INDEX_BITS)));
    }
